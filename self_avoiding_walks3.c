...
const int n = 10;
const double eps = 0;
double estimated_SAWs[10000000];
int hist[2][200];
...
double generate_child(int grid[n+3][n+3], int x, int y, int n_steps, double g_x, double wt_sum, FILE *f_1) {
  double *dir_pr;
  int dir;
  const int dim = n + 3;
  ...
  wt_sum = wt_sum + (1 / g_x / 5);
  gen_walks++;
  estimated_SAWs[gen_walks] = wt_sum / gen_walks;
  fprintf(f_1, "%e %d %e\n", estimated_SAWs[gen_walks], n_steps, 1 / g_x / 5);

  return wt_sum;
}

double generate_SAWs(int M, int n, FILE *f_1, FILE *f_2)
{
  srand(0);
  ...
  while (gen_walks < M) {
    ...
    while (x > 0 && x < last_i && y > 0 && y < last_i) {
      ...
      if (n_steps > checkpoint) {
        ...
        while (child < 5) {
          ...
          wt_sum = generate_child(temp, x, y, n_steps, g_x, wt_sum, f_1);
          child++;
          if (gen_walks >= M) {
            break;
          }
    ...
    if (n_steps <= checkpoint) {
      ...
      gen_walks++;
      estimated_SAWs[gen_walks] = wt_sum / gen_walks;
      fprintf(f_1, "%e %d %e\n", estimated_SAWs[gen_walks], n_steps, 1 / g_x);
    }
  }
...
int main(void)
{
  FILE *f_1 = fopen("design_3_saws.txt", "w");
  FILE *f_2 = fopen("design_3_longest_path.txt", "w");
  printf("SAWs: %e \n", generate_SAWs(M, n, f_1, f_2));
  fclose(f_1);
  fclose(f_2);
  getchar();
  return 0;
